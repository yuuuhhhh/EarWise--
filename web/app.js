...
(() => {
  ...
  function renderRegistration() {
    $("#main-content").innerHTML = `<section class="card"><div class="card-body">
      <div class="section-title"><span class="section-number">01 /</span><h2>被试登记</h2></div><p class="subtitle">每次完成一个 round；可选择任意合法编号进行补采。</p>
      <form id="registration-form"><div class="form-grid">
        ...
        <label class="field"><span>轮次 · Round<span class="required">*</span></span><select id="round" required><option value="" disabled ${!draft.round ? "selected" : ""}>请选择</option>${[1,2,3,4].map((n)=>`<option value="${n}" ${String(draft.round)===String(n)?"selected":""}>第 ${n} 轮</option>`).join("")}</select></label>
      ...
      <div class="registration-divider"></div><div class="section-title"><span class="section-number">02 /</span><h2>本轮计划与准备</h2></div><p class="subtitle">30 秒基线 → 视频与量表 × 2 → 保存校验</p><div id="trial-plan" class="empty-plan">完成上方登记后，查看本轮视频与顺序。</div>
  ...
  function renderVideo() {
    ...
    $("#main-content").innerHTML = `<section class="card video-card"><div class="phase-header"><div><h2>${esc(conditionLabels[trial.condition] || "实验视频")}</h2><p class="subtitle">${esc(trial.video.path || trial.video.video_id)} · 按原始速度完整播放</p></div><span class="phase-badge">Trial ${esc(trial.trial_order)} / 2</span></div><div class="video-stage"><video id="experiment-video" preload="auto" playsinline disablepictureinpicture disableremoteplayback tabindex="-1" aria-label="当前实验视频"></video><div id="video-overlay" class="video-overlay"><p id="video-overlay-text">正在准备视频…</p><button id="play-button" class="button" hidden>点击开始播放</button></div></div><div class="video-footer"><span id="play-status" class="playing-label">等待实际播放</span><span>播放结束后进入本段视频的量表</span></div></section>`;
