...
本项目实现中文本地采集页面、耳机蓝牙预览、30 秒基线、两个视频 trial、两次主观量表以及每次尝试独立的三文件保存。现有 16 个视频和原始参考脚本保留在原位置。基线引导已按实验负责人确认设置为 **“静息30s”**。
...
1. 输入被试编号（`001` 会保留前导零），选择 day 1–3 和 round 1–4。系统显示本轮视频和既往尝试，可选任意合法 day/round 补采。
...
5. 选择程度、确信度两个 1–5 整数后提交。随后播放另一个条件的视频，再填写第二次量表。
6. 等待保存校验；只有显示“已完成，数据已保存”才算正常完成。结果页列出本次目录和三个文件。

奇数 round 顺序为 attention → 量表 → relax → 量表；偶数 round 相反。每轮仅在开始时有一次基线。基线到视频的过渡、缓冲、量表期间继续保存 EEG。已有完成记录需要明确确认重采，每次尝试使用新目录，不覆盖旧数据。
...
`config/video_manifest.json` 固定包含 24 个 trial，每个 trial 恰好一个视频。attention 编号按 `(day−1)×4+round`；relax 编号等于 round，跨三天复用。启动和预检会校验完整映射，缺失或错配会报出具体文件，不以临时示例素材替代。
...
`eeg_raw.csv` 保存主机接收事实，接收时刻不是设备物理采样时刻，连续行号不是无缺失生理样本编号。`labels.csv` 记录阶段边界、前端播放事件及两次评分；前端通过往返校时映射到后端单调时间，仍是软件时间标记，未测量屏幕物理呈现延迟。阶段区间按 `[开始,结束)` 解释，不能直接把前端 `performance.now()` 与后端时钟相减。
