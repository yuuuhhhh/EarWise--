...
EXPERIMENT_PROTOCOL_ID = "earwise-3round-4trial-randomstart-v4"
TRIALS_PER_ROUND = 4
TOTAL_ROUNDS = 3
...
def _round(round):
    if type(round) is not int or round not in range(1, TOTAL_ROUNDS + 1):
        raise ConfigurationError("round 必须为整数 1、2 或 3")
...
def validate_manifest(root: Path) -> list[dict]:
    ...
    if not isinstance(manifest, dict) or manifest.get("schema_version") != "4.0":
        raise ConfigurationError("video_manifest.json 必须为 schema_version=4.0 的对象")
    entries = manifest.get("trials")
    if not isinstance(entries, list) or len(entries) != 12:
        raise ConfigurationError("视频清单必须包含 12 个 trial，覆盖 3 个 round，每轮 4 个 trial，每 trial 一个视频")
    validated, seen = [], set()
    for entry in entries:
        if not isinstance(entry, dict):
            raise ConfigurationError("视频清单 trial 必须为对象")
        if "day" in entry:
            raise ConfigurationError("视频清单不再使用 day 字段，请使用 round 1、2、3")
        round, condition = entry.get("round"), entry.get("condition")
        trial_order = entry.get("trial_order")
        _round(round)
        if type(trial_order) is not int or trial_order not in range(1, TRIALS_PER_ROUND + 1):
            raise ConfigurationError(f"round {round} 的 trial_order 必须为整数 1、2、3 或 4")
        key = (round, trial_order)
        if key in seen:
            raise ConfigurationError(f"视频清单重复：round {round} trial_order {trial_order}")
        seen.add(key)
        # Manifest order describes each round's fixed pool, not presentation order.
        expected_condition = "attention" if trial_order % 2 else "relax"
        if condition != expected_condition:
            raise ConfigurationError(f"round {round} trial_order {trial_order} 的 condition 必须为 {expected_condition}")
        pair_index = (trial_order - 1) // 2 + 1
        index = (round - 1) * 2 + pair_index if condition == "attention" else pair_index
        ...
        if not isinstance(video, dict) or video.get("path") != relative or video.get("video_id") != video_id:
            raise ConfigurationError(f"round {round} {condition} 必须对应 {relative}（ID {video_id}），每 trial 恰好一个视频")
        ...
        validated.append({"round": round, "trial_order": trial_order, "condition": condition,
                          "video": {"video_id": video_id, "path": relative,
                                    "filename": video["filename"], "url": "/media/" + quote(relative)}})
    expected = {(round, trial_order) for round in range(1, TOTAL_ROUNDS + 1)
                for trial_order in range(1, TRIALS_PER_ROUND + 1)}
    if seen != expected:
        raise ConfigurationError("视频清单必须完整覆盖 3 个 round、每轮 4 个 trial")
    return validated
...
def randomization_for(subject_id: str, round: int) -> dict:
    """Reproducible random first condition; retries preserve the round's order."""
    subject = validate_subject(subject_id).casefold()
    _round(round)
    identity = json.dumps([EXPERIMENT_PROTOCOL_ID, subject, round], ensure_ascii=False, separators=(",", ":"))
    seed = hashlib.sha256(identity.encode("utf-8")).hexdigest()
    return {"method": "sha256-subject-round-first-condition-v1", "seed": seed,
            "first_condition": "attention" if int(seed[-1], 16) % 2 == 0 else "relax",
            "retry_policy": "reuse-subject-round"}


def plan_fingerprint(subject_id: str, round: int, plan: list[dict]) -> str:
...
def plan_for(root: Path, round: int, *, subject_id: str) -> list[dict]:
    _round(round)
    randomization = randomization_for(subject_id, round)
